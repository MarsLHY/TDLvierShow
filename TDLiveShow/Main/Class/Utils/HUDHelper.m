...
{
    UIView *inView = view ? view : [TDBaseAppdelegate sharedAppDelegate].window;
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIView *inView = [TDBaseAppdelegate sharedAppDelegate].window;
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:inView];
        if (![NSString isEmpty:msg])
        {
...
- (void)tipMessage:(NSString *)msg delay:(CGFloat)seconds completion:(void (^)())completion
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        MBProgressHUD *hud = [[MBProgressHUD alloc] initWithWindow:[TDBaseAppdelegate sharedAppDelegate].window];
        [[TDBaseAppdelegate sharedAppDelegate].window addSubview:hud];
        hud.mode = MBProgressHUDModeText;
        hud.labelText = msg;
        [hud show:YES];
        [hud hide:YES afterDelay:seconds];
